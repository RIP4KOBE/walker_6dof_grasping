...
import pybullet
import math
from gpn.grasp import Label
from gpn.perception import *
from gpn.utils import btsim, workspace_lines
...
class ClutterRemovalSim(object):
    def __init__(self, scene, object_set, gui=True, seed=None):
        ...
        self.backend = self.world.p
        self.gripper = Gripper_walker(self.world)
        self.size = 6 * 0.06
    ...
    def reset(self, object_count):
        ...
        if self.gui:
            self.world.p.resetDebugVisualizerCamera(
                ...
                cameraYaw=-90.0,
                cameraPitch=-45,
                cameraTargetPosition=[0.20, 0.20, -0.1],
            )
    ...
    def place_table(self, height):
        urdf = self.urdf_root / "setup" / "plane-lz.urdf"
    ...
    def generate_packed_scene(self, object_count, table_height):
        ...
        while self.num_objects < object_count and attempts < max_attempts:
            ...
            z = table_height + 0.5 * (upper[2] - lower[2])
        ...
        if self.gripper.detect_contact():
            ...
            if self.gripper.detect_contact() and not allow_contact:
                ...
                if self.check_success(self.gripper):
                    result = Label.SUCCESS, self.gripper.max_opening_width
                    if remove:
...
class Gripper_walker(object):
    """Simulated Walker2 humanoid hand."""
    def __init__(self, world):
        self.world = world
        self.urdf_path = Path("/data/urdfs/walker2_hand/Hand_with_friction.urdf")
        self.max_opening_width = 0.08
        self.finger_depth = 0.02
        R = Rotation.from_euler("xyz", [-0.9,PI/2,0])*Rotation.from_euler("xyz", [0,0,PI/2])
        self.T_body_tcp = Transform(R, [0.12, 0.03, -0.02])
    ...
    def move(self, width):
        ...
        for i in range(1, steps_to_take):
            for j in range(0, numJoints):
                if j % 2 == 1:
                    angle = 1.1 * i / steps_to_take
                    # print('\r' + str(angle), end='')
                    p.setJointMotorControl2(bodyUniqueId=self.body.uid,
                                            jointIndex=j,
    ...
    def read(self):
        joint_state1 = self.world.p.getJointState(self.body.uid, 1)[0]
        # print("joint_state1",self.world.p.getJointState(self.body.uid, 1))
        joint_state2 = self.world.p.getJointState(self.body.uid, 3)[0]
        # print("joint_state2",self.world.p.getJointState(self.body.uid, 3))
        width = joint_state1 + joint_state2
        print("width",width)
        return width


class Gripper_walkerX(object):
    """Simulated WalkerX humanoid hand."""
    def __init__(self, world):
        self.world = world
        self.urdf_path = Path("data/urdfs/walkerx_rhand/hand.urdf")
        # self.urdf_path = Path("data/urdfs/walkerx_rhand/hand_no_mesh.urdf")
        self.max_opening_width = 0.08
        self.finger_depth = 0.0
        R = Rotation.from_euler("xyz", [PI/2, 0 , 0.85]) #* Rotation.from_euler("xyz", [0,0,PI/2])
        self.T_body_tcp = Transform(R, [0.04, -0.14,0.02])
        self.T_tcp_body = self.T_body_tcp.inverse()
    ...
    def move_tcp_xyz(self, target, eef_step=0.002, vel=0.01, abort_on_contact=True):
        T_world_body = self.body.get_pose()
        T_world_tcp = T_world_body * self.T_body_tcp
        ...
        dur_step = np.linalg.norm(diff) / vel
    ...
    def open(self):
        ...
        for i in range(1, steps_to_take):
            for j in range(1, numJoints):
                angle = angles[j] * i / steps_to_take
                p.setJointMotorControl2(bodyUniqueId=self.body.uid,
                                        jointIndex=j,
                                        controlMode=p.POSITION_CONTROL,
                                        targetPosition=angle,
