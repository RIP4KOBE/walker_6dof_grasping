...
def draw_grasp(grasp, score, finger_depth):
    ...
    pubs["grasp_vgn"].publish(MarkerArray(markers=markers))
...
def draw_grasps(grasps, scores, finger_depth):
    ...
    pubs["grasps_vgn"].publish(msg)


def clear():
    pubs["workspace_vgn"].publish(DELETE_MARKER_MSG)
    pubs["tsdf_vgn"].publish(ros_utils.to_cloud_msg(np.array([]), frame="task"))
    pubs["points_vgn"].publish(ros_utils.to_cloud_msg(np.array([]), frame="task"))
    clear_quality()
    pubs["grasp_vgn"].publish(DELETE_MARKER_ARRAY_MSG)
    clear_grasps()
    pubs["debug_vgn"].publish(ros_utils.to_cloud_msg(np.array([]), frame="task"))


def clear_quality():
    pubs["quality_vgn"].publish(ros_utils.to_cloud_msg(np.array([]), frame="task"))


def clear_grasps():
    pubs["grasps_vgn"].publish(DELETE_MARKER_ARRAY_MSG)


def _create_publishers():
    pubs = dict()
    pubs["workspace_vgn"] = Publisher("/workspace_vgn", Marker, queue_size=1, latch=True)
    pubs["tsdf_vgn"] = Publisher("/tsdf_vgn", PointCloud2, queue_size=1, latch=True)
    pubs["points_vgn"] = Publisher("/points_vgn", PointCloud2, queue_size=1, latch=True)
    pubs["quality_vgn"] = Publisher("/quality_vgn", PointCloud2, queue_size=1, latch=True)
    pubs["grasp_vgn"] = Publisher("/grasp_vgn", MarkerArray, queue_size=1, latch=True)
    pubs["grasps_vgn"] = Publisher("/grasps_vgn", MarkerArray, queue_size=1000, latch=True)
    pubs["debug_vgn"] = Publisher("/debug_vgn", PointCloud2, queue_size=1, latch=True)
    return pubs
