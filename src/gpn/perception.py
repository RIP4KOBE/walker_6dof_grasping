...
from gpn.utils.transform import Transform
...
def camera_on_sphere(origin, radius, theta, phi):
    #convert the camera coordinates from spherical coordinate system to rectangular coordinate system
    eye = np.r_[
    ...
    # the transform from task to the cam(T_Cam_task)
    return Transform.look_at(eye, target, up) * origin.inverse()#T_cam_task = T_cam_WorkspaceCenter * T_WorkspaceCenter_task(origin.inverse())
